...
A baseline-vs-current comparison is only valid if both runs evaluated the same dataset version. If the dataset changed between the baseline run and the PR run, the delta conflates two causes — the code change and the dataset change — and the gate becomes noise. Pin the version per run, record it in the results, and fail fast when a run is unpinned or the two versions disagree.

```bash
# Fail the job before interpreting any delta if versions do not match
BASELINE_VERSION=$(jq -r '.datasetVersion // "unpinned"' baseline.json)
CURRENT_VERSION=$(jq -r '.datasetVersion // "unpinned"' current.json)

if [ "$CURRENT_VERSION" = "unpinned" ]; then
  echo "Eval ran against an unpinned dataset. Refusing to gate on it."
  exit 1
fi

if [ "$BASELINE_VERSION" != "$CURRENT_VERSION" ]; then
  echo "Dataset version mismatch: baseline=$BASELINE_VERSION current=$CURRENT_VERSION"
  echo "A dataset change is not a regression. Rebaseline before comparing."
  exit 1
fi
```

Deciding _what_ goes into that versioned dataset — sourcing, labeling, leakage checks, and re-sampling as production drifts — is covered in [Datasets and experiments](/docs/testing/datasets-and-experiments).
...
console.log(
  `Pass rate: ${baseline.summary.pass_rate.toFixed(2)} → ${current.summary.pass_rate.toFixed(2)} (${passRateDelta > 0 ? "+" : ""}${passRateDelta.toFixed(1)}%)`,
);
console.log(
  `Tool F1: ${baseline.summary.tool_f1.toFixed(2)} → ${current.summary.tool_f1.toFixed(2)} (${f1Delta > 0 ? "+" : ""}${f1Delta.toFixed(1)}%)`,
);
console.log(
  `Cost: $${baseline.summary.cost.toFixed(2)} → $${current.summary.cost.toFixed(2)} (${costDelta > 0 ? "+" : ""}${costDelta.toFixed(1)}%)`,
);
...
- Statistical power for evals: [Anthropic, "Demystifying evals for AI agents"](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents)

## See also

- `/docs/testing/datasets-and-experiments` — curating and versioning the dataset these gates compare against
