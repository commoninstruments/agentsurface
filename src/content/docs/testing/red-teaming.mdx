...
Red-teaming evaluates agent resilience against malicious inputs. Attacks range from prompt injection to data exfiltration loops to tool abuse. Unlike evals on happy-path tasks, red-team suites measure failure modes that require explicit testing. This is the offense side; [Runtime Guardrails](/docs/agents/runtime-guardrails) is the defense side that stands in the production request path.
