...
Model-based grading for open-ended outputs with no deterministic ground truth (plan quality, semantic correctness, summary coherence). LLM judges can prefer outputs from themselves or similar models, exhibit position bias (prefer first/last), and reward verbosity. Mitigations: use a judge from a different model family, hybrid grading (code + model), and calibration against human-labeled baselines. Use code-based graders for deterministic outputs (tool routing, DB state verification).
...
A judge model tends to prefer outputs from itself or similar models. If you use GPT-5.4 to judge outputs from Claude Opus 4.8, the judge may systematically overrate outputs that look like GPT-style reasoning.

**Mitigation:** Use a judge from a different model family and calibrate it against human-labeled examples. For pairwise comparisons, run at least some cross-provider checks so style similarity does not become a hidden scoring advantage.
...
function cohensKappa(humanLabels: number[], judgeLabels: number[]): number {
  const po = // Observed agreement
    humanLabels.reduce((sum, h, i) => sum + (h === judgeLabels[i] ? 1 : 0), 0) / humanLabels.length;
...
async function hybridGrader(
  ...
  const typesCorrect = Object.entries(toolCall.arguments).every(([key, value]) => {
...
- **Use cheaper judges for pre-screening.** Haiku 4.5 for initial triage, then Opus 4.8 for borderline cases.
