...
The extensions above — especially `x-speakeasy-mcp` — feed directly into tools that generate MCP servers from your OpenAPI spec. Generation is only a starting point: a naive 1:1 mapping produces 50–300 tools that overwhelm agents, with descriptions written for human developers rather than agent reasoning. The correct workflow is generate → prune → rewrite → add composites, targeting 10–30 tools per domain.

For the full generator comparison (Stainless, Speakeasy, FastMCP, openapi-mcp-generator), the hybrid workflow step-by-step, and the reasoning behind not shipping generated output directly, see [Auto-Generating MCP from OpenAPI](/docs/mcp-servers/auto-generation). This page owns the extension mapping that those generators consume; that page owns the generation workflow.
