...
Idempotency keys are client-generated identifiers that prevent double-submission of write operations. If a request is retried due to network failure, timeout, or connection loss, the server recognizes the same logical operation and returns the cached response instead of executing twice. The client sends an `Idempotency-Key` header alongside its `Authorization` and `DPoP` headers; the server caches only successful (2xx) responses for a window — typically 24 hours — so failed requests stay retriable. Generate keys as UUIDv7 so the embedded timestamp lets old keys expire naturally, and log them for audit trails.

The full key-generation, storage, and TTL mechanism — including the production Redis handler — lives on the [Idempotency](/docs/error-handling/idempotency) page. What matters at the authentication layer is how this mechanism composes with DPoP replay protection, below.
...
async function handlePayment(req: Request): Promise<Response> {
  ...
  await redisClient.set(`idempotency:${idempotencyKey}`, JSON.stringify(result), {
    EX: 24 * 60 * 60,
  });
