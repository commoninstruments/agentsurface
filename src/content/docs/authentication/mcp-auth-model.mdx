...
MCP authorization and `auth.md` solve adjacent problems:

- MCP authorization describes how an MCP client obtains and presents an access token to a protected HTTP MCP server.
- `auth.md` describes how an agent can register or claim a user-bound credential with a service before making authenticated calls.

Use MCP authorization for the transport-level OAuth flow. Use `auth.md` only when the service also wants to publish an agent-readable registration path, such as provider-attested ID-JAG registration or an OTP-backed user claim ceremony.
...
- [MCP Authentication](/docs/mcp-servers/authentication) — practical server-implementer guide: bearer tokens, identity proxies, PKCE, JWT validation, and scope enforcement
...
- [auth.md](/docs/authentication/auth-md)
