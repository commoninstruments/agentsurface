...
Default: claude-opus-4-8 (best for complex analysis and coding)
...
Use claude-sonnet-5 for fast iteration on simple tasks.
...
Default: claude-opus-4-8 (complex analysis)
Use claude-sonnet-5 for: fast iteration, simple refactoring
...
- Skill auto-trigger only for _directly_ matching files (new tests → auto /review doesn't fire on library changes)
```
