...
AGENTS.md is documented at [agents.md](https://agents.md). It is plain markdown with no schema—no YAML frontmatter, no JSON. This simplicity is intentional: agents must parse it with regex and heuristics, so readability and structural consistency matter more than strict formats.

AGENTS.md is read by multiple coding-agent and IDE tools, including Claude Code, Cursor, Windsurf, Kilo, GitHub Copilot, Aider, Jules, Zed, Warp, and RooCode.

## Discovery and Precedence

Agents discover `AGENTS.md` by walking up the directory tree from the current working directory to the repository root. When multiple files exist, the one closest to the working directory wins:
...
This lets you set repository-wide defaults and override them for specific subsystems — the authentication module can carry stricter modification rules while test fixtures stay permissive. Keep nested files scoped to what genuinely differs; see [Monorepos](/docs/context-files/monorepos) for per-package patterns.

The maximum file size is 32 KiB. Files larger than that may be truncated or ignored by some agents, which is another reason to keep the root file tight and push detail into nested files or linked docs.
...
Web (Next.js 16), shared type definitions. Handles time-series data
...
Longer is not better. Research from ETH Zurich found that hand-curated `AGENTS.md` files improved agent task completion by roughly +4%, while LLM-generated files reduced it by roughly -2% at a +20% token cost. If you are over the line limit, the file probably contains a codebase overview or a style guide — both anti-patterns. Move style rules to lint config where they are enforced automatically, and write context files by hand.
...
SaaS analytics platform: API (Fastify + Node), Web (Next.js 16), Shared types.
