...
Tool annotations are metadata hints signaling tool behavior to clients. Current MCP tool hints include readOnlyHint, destructiveHint, idempotentHint, and openWorldHint. Annotations are hints, not guarantees or permission policy. Unverified servers' annotations are treated as untrusted. Verified, trusted servers can enable smarter client behavior, but destructive and high-risk tools still need server-side approval and authorization policy.
...
- openWorldHint: tool interacts with external systems outside the server boundary
...
Annotations do not enforce policy. They do not prove that a tool is safe, read-only, idempotent, or internal-only. They only tell a client what the server claims about a tool.

For production systems:

- enforce user, tenant, and scope authorization in trusted server code
- require approval for destructive, authenticated, production, browser, sandbox, package-install, payment, email-send, and file-write tools
- log the annotation set and the actual approval decision
- default unknown tools to high-risk behavior
- pin or trust remote servers before relying on their metadata for automation
...
<Callout type="info">
  Annotations are part of the tool definition and are returned in the `tools/list` response. Clients
  read them once during capability negotiation and cache them for the session. If you change an
  annotation on a running server, connected clients will not see the change until they re-fetch the
  tool list.
</Callout>

## Related

- [Tool Annotations (READ_ONLY/WRITE/DESTRUCTIVE)](/docs/cookbook/tool-annotations) — applied registry pattern: presets driving system prompts, client UI, and tool selection
- [Idempotency and Safety](/docs/tool-design/idempotency-and-safety) — designing tools to be safely retryable and clearly classified
- [MCP Specification 2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25) — the tool annotation definitions
