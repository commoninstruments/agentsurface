...
<Callout type="warn">
  Only accept identity headers from a trusted proxy IP range. If any client can send `X-User-Id:
  admin`, your authorization is meaningless. Configure your server to only accept these headers from
  the proxy's known IP addresses.
</Callout>
...
The protocol-level model — protected-resource metadata discovery, authorization-server discovery, client registration options, and token audience binding — lives in [MCP Authorization](/docs/authentication/mcp-auth-model). This section is the server-implementer view: the endpoints you expose, PKCE, token validation, scope enforcement, and the on-behalf-of flow. Read the canonical page first if you need the full OAuth 2.1 discovery and registration flow; use this page to implement it.

### Required .well-known Endpoints

Your server must expose two discovery endpoints so clients can discover your authorization server (the [MCP Authorization](/docs/authentication/mcp-auth-model) and [Protected Resource Metadata](/docs/authentication/protected-resource-metadata) pages cover the discovery protocol and full field set):
...
export async function validateToken(token: string): Promise<{ userId: string; scopes: string[] }> {
      ...
      content: [
        {
          type: "text",
          text:
            "Insufficient permissions. The billing:write scope is required to create invoices. " +
...
```typescript
async function getDownstreamToken(userToken: string, targetAudience: string): Promise<string> {
...
This preserves the audit trail — the downstream API sees that the request is `on_behalf_of` the original user, rather than only your service identity.
...
export const { GET, POST, DELETE } = experimental_withMcpAuth(mcpHandler, validateAuth);
...
<Callout type="warn">
  HTTPS is required for all OAuth flows. The `code_challenge` mechanism of PKCE protects the
  authorization code in transit, but the access token itself is a bearer credential — anyone who
  intercepts it can use it. Never run an OAuth-authenticated MCP server over plain HTTP in
  production.
</Callout>

## Related

- [MCP Authorization](/docs/authentication/mcp-auth-model) — canonical protocol flow: protected-resource metadata, authorization-server discovery, client registration, and token audience binding
- [Protected Resource Metadata](/docs/authentication/protected-resource-metadata) — the `/.well-known/oauth-protected-resource` document your server publishes
- [OAuth 2.1 for Agents](/docs/authentication/oauth-for-agents) — the underlying grant flow
