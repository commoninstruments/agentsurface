...
The `x-speakeasy-mcp` extension works with Speakeasy's generator and can be added to any OpenAPI spec as a vendor extension. This section covers how it drives the generation workflow; for the full extension surface — tool-name overrides, scope grouping, parameter-description overrides, and pattern-based scope mapping — see [Semantic Extensions for Agent APIs](/docs/api-surface/openapi-extensions). You can annotate your spec incrementally, adding MCP configuration to the operations you want to expose without modifying the spec's existing structure:
...
<Callout type="warn">
  Auto-generation produces a starting point, not a finished MCP server. Shipping generated output
  without the pruning, rewriting, and composite-tool phases will result in a server that technically
  works but that agents use poorly — wrong tool selection, missing context, redundant calls. The
  quality of your MCP server is determined by the rewriting phase, not the generation phase.
</Callout>
...
- [Semantic Extensions for Agent APIs](/docs/api-surface/openapi-extensions) — the `x-speakeasy-mcp` and `x-action` extension mapping these generators consume
- [OpenAPI for Agents](/docs/api-surface/openapi-for-agents) — the base spec design that generation reads from
- [Tool Annotations](/docs/mcp-servers/annotations) — the behavioral hints generated tools should carry
