...
<Callout type="warn">
  Approval workflows require durable state storage. If the process restarts between suspend and
  resume, the workflow state must survive. Use a persistent checkpointer (PostgreSQL, Redis,
  Upstash) rather than in-memory storage for any production HITL flow.
</Callout>
