...
Tool selection accuracy degrades with set size. This is not a hypothetical concern — it is measurable. OpenAI's function calling documentation notes degradation around 10 tools per context. Use 20 tools as a review threshold: above that, introduce namespacing, per-turn selection, or plugin-level loading.
...
When a large tool surface is unavoidable, dynamic selection keeps each context small by activating only the tools relevant to the current task rather than loading all of them — this is what makes a shared 100-tool registry usable without any single agent's context exceeding the ~20-tool ceiling. The main techniques — BM25 keyword search, embedding-based similarity ranking, the Vercel AI SDK's `activeTools` per-turn filtering, and MCP's `notifications/tools/list_changed` for reactive, context-sensitive tool sets — are explained with code in [Tool Curation](/docs/tool-design/tool-curation). The production embedding-based walkthrough (the `toolpick` library, the `prepareStep` hook, and embedding caches) is in [Semantic Tool Selection at Scale](/docs/cookbook/semantic-tool-selection).
...
- [Tool Design](/docs/tool-design) — the cross-framework principles for tool definition quality
