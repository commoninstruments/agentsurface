...
This page covers the five primary patterns, how the major frameworks implement them, and the decision rules for choosing between them.
...
A reliable production default is a hybrid: a deterministic workflow graph handles control flow, error recovery, and state transitions, while AI agents are invoked only at specific decision points where judgment is required.
...
<Callout type="info">
  Fumadocs and other framework docs use a similar pattern: static routing for well-known paths,
  dynamic generation only where the content cannot be predetermined. The same principle applies to
  agent orchestration.
</Callout>
