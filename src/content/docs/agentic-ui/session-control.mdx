...
Modes should be explicit, visible, and switchable mid-session. The user should always be able to see which mode is active and downgrade a running agent from execute to draft. Auto-accept is a mode, not a default — surface it as a deliberate choice.
...
The failure to avoid is a thread that becomes unreadable because five subagents interleave their raw output into one stream. Surface delegation as structure, not as noise.

## Tool Approvals

Tool approval is where session control meets safety. This page covers the UI and controller patterns; for where to place gates and how suspend/resume works underneath, see [Human-in-the-Loop](/docs/multi-agent/human-in-the-loop), which owns the approval-checkpoint semantics.
...
- **Per-session policy** — "approve this tool for the rest of the session" or "always ask for this tool." The decision is remembered for the thread's lifetime, not baked into code.
...
Approvals are a state machine, not a modal that disappears on refresh. Persist explicit states server-side so a reload or reconnect restores exactly where the run paused:
...
Steering works best when the underlying run has checkpoints. If all state lives in a single model transcript, the controller cannot cleanly splice a new instruction into a run or resume from the right point after a correction. The runtime side of this — replay-safe activities, checkpointing partial state, injecting signals into a live workflow — belongs to [Durable Execution](/docs/runtime-boundaries/durable-execution). Session control owns the UX: how the injection is captured, shown, and acknowledged; durable execution owns how the running workflow absorbs it.

The event-protocol angle is worth tracking here. CopilotKit's AG-UI is the current de facto layer for streaming agent state and steering events to a UI across vendors; see the [Emerging Standards Watchlist](/docs/protocols/emerging-standards) for its status.
...
- [Human-in-the-Loop](/docs/multi-agent/human-in-the-loop) — approval-checkpoint semantics and framework suspend/resume
...
- [Runtime Boundaries](/docs/runtime-boundaries) — where durable execution and background work live
