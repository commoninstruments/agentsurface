...
description: Stop building custom orchestration. Use ToolLoopAgent, generateText stopWhen, or Claude Code SDK. The loop is solved.
---

## Summary

The "agentic loop" — request user message, call tool, stream response, repeat until done — is a commodity problem solved by modern AI frameworks. Vercel AI SDK `generateText`/`streamText` with `stopWhen`, Claude Code SDK `query()`, Claude Managed Agents, and OpenAI Agents SDK all handle state, streaming, tool execution, and max-step limits. The value is in tools, prompts, and error handling, not in reinventing control flow.

- **Vercel AI SDK**: `generateText`/`streamText` with `stopWhen`; integrates with many providers and app-route streaming.
- **Claude Code SDK**: `query()` method; strong fit for coding/repo agents, MCP tools, and permissioned tool execution.
- **Claude Managed Agents**: managed sessions, containers, Skills, vault credentials, memory, webhooks, outcomes, and multiagent sessions.
- **OpenAI Agents SDK**: OpenAI-native agents, handoffs, hosted tools, tracing/evals, and remote MCP.
...
- **`stopWhen`**: Predicate that receives step count; return `true` to halt. Start with a small cap such as 5-10 steps, then tune from traces.
- **`prepareStep`**: Hook to select which tools are exposed per turn (enables semantic selection).
- **`onFinish`**: Guaranteed cleanup callback — runs whether the agent succeeds or fails. See [Resource Lifecycle](#resource-lifecycle) below.
...
## Pattern: Claude Code SDK query()

Use this when you want Claude Code's harness programmatically for repository work, coding tasks, local automation, or MCP-backed tools.
...
- Reuses Claude Code's tool harness and permission model.
- Works naturally with local/repo context and MCP configuration.
- Good for coding, incident response, migration, and agentic CLI workflows.

**Cons**:

- Fit is weaker for product chat UI than Vercel AI SDK.
- App-specific tools are usually exposed through MCP rather than direct in-process handlers.
- Permission and allowed-tool policy still need explicit configuration.
...
**Framework difference**: Vercel AI SDK uses Zod schemas and `execute` handlers. Claude Code SDK uses JSON Schema via `input_schema` (tools execute server-side). OpenAI Agents may return tool calls for client-side execution.
...
- Use Vercel AI or Claude Code SDK.
...
- [ ] Choose framework: Vercel AI (recommended), Claude Code SDK, or OpenAI Agents.
