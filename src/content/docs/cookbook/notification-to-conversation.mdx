...
await storeNotificationContext(userId, {
  ...
  timestamp: Date.now(),
  suggestedPrompts: ["Show me them", "Which ones need receipts?", "Categorize them"],
...
```typescript
function formatNotificationContextForPrompt(context: NotificationContext | null): string {
  if (!context) return "";
...
```typescript
function isWithinWhatsAppSessionWindow(lastUserMessageAt: Date | null): boolean {
...
export async function handleWhatsAppReply(message: WhatsAppMessage) {
  ...
  const systemPrompt = await buildSystemPromptWithNotificationContext(userCtx, normalized.userId);
...
export async function getNotificationContextFromDb(
  userId: string,
