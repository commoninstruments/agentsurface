...
- **Threshold**: Past ~20 tools, prefer per-turn selection over pre-loading — see [Tool Curation](/docs/tool-design/tool-curation) for why accuracy degrades and what other strategies exist.
- **~12 tools per turn**: Good starting point; tune against your tool set, model, and evals.
...
**Cost**: ~500 tokens listing tool definitions alone, and the model wastes context parsing irrelevant tools (order tools when asking about customers). Why accuracy degrades past ~20 tools, and the role-based and keyword strategies that complement embeddings, live in [Tool Curation](/docs/tool-design/tool-curation) — this recipe is the applied embedding-based implementation.
...
- **Coverage**: If a tool isn't in the top 12, the model can't use it directly. Mitigation: `search_tools` meta-tool lets model _search_ for tools dynamically.
...
server.registerTool(
  "search_tools",
  {
    ...
    description:
      "Search for a tool by name or capability. Use this when you can't find the tool you need.",
    inputSchema: z.object({
      query: z
        .string()
        .describe("What do you want to do? (e.g., 'list reports', 'send email', 'delete invoice')"),
    }),
...
- [Naming and Descriptions](/docs/tool-design/naming-and-descriptions) — tool description best practices (embedding quality depends on description quality).
