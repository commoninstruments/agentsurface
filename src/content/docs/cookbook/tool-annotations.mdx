...
MCP tool definitions support an `annotations` object with four hints — `readOnlyHint`, `destructiveHint`, `idempotentHint`, and `openWorldHint`:
...
The semantics of each hint, their conservative defaults, and how clients act on them are covered in [Tool Annotations](/docs/mcp-servers/annotations). This page assumes them and focuses on the applied registry pattern: collapsing the four hints into three presets and driving system prompts, client UI, and tool selection from them.
...
- [Tool Annotations](/docs/mcp-servers/annotations) — canonical semantics of the four MCP hints and client-behavior implications.
