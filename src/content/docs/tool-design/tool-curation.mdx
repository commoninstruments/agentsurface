...
Agent frameworks route tool calls by semantic similarity between the user's request and each tool's description, so every extra tool raises the chance of routing to the wrong one. OpenAI recommends fewer than 10 tools per agent session; selection accuracy degrades sharply past a hard ceiling of about 20. If your surface is larger than that, group, filter, and compose rather than exposing everything.
...
BM25 matches on keywords; embedding-based selection ranks tools by semantic similarity between the user's message and each tool description, catching relevant tools even when the wording differs. Embed every tool description once at startup, cache the vectors, then at each step select the top candidates by cosine similarity — roughly 12 per turn is a good starting point, tuned against your tool set and evals. Pair it with a small "always-active" set of critical tools (web search, a `search_tools` meta-tool) that bypass ranking, so the agent can always discover what selection missed.

The applied production walkthrough — the `toolpick` library, the Vercel AI SDK `prepareStep` hook, embedding caches, and cross-tool dependency maps — is in [Semantic Tool Selection at Scale](/docs/cookbook/semantic-tool-selection).
...
const adminAgent = agent({
  ...
  model: "claude-opus-4-8",
  requiresHumanApproval: (toolName) => toolRegistry[toolName].category === "dangerous",
});
```
...
export function selectToolsForBeta(allTools: ToolMetadata[]): ToolMetadata[] {
  return allTools.filter((t) => t.stability === "stable" || t.stability === "beta");
}
```
...
export function listToolsByPurpose(purpose: string): ToolMetadata[] {
  ...
  return descriptions.filter((t) => t.relevance > 0).map(({ relevance, ...t }) => t);
}
