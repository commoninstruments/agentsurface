...
| ACP (commerce) | Agent → merchant       | Beta       | Agent initiates purchases     |
...
<Callout type="warn">
  "ACP" below refers to the **Agentic Commerce Protocol** (OpenAI + Stripe). The acronym is
  overloaded — the unrelated **Agent Client Protocol** (Zed/JetBrains editor-to-agent standard, at
  stable protocol version 1) and IBM's dead Agent Communication Protocol also used it. See the [ACP
  page](/docs/protocols/acp) for the full disambiguation.
</Callout>
...
| **A2A v1.0.1**     | Agent-to-agent delegation     | Horizontal (peers)            | JSON-RPC, gRPC, HTTP+JSON       | OAuth 2.1 + DPoP                | `/.well-known/agent.json`       | Production |
| **ACP (commerce)** | Agent-initiated commerce      | Horizontal (agent → merchant) | HTTPS REST with SSE             | Bearer token + request signing  | `/.well-known/acp.json`         | Beta       |
...
ACP (the Agentic Commerce Protocol, OpenAI + Stripe) is a Beta open commerce standard. It is appropriate for controlled pilots where agent-initiated purchases need explicit payment boundaries. Momentum has cooled since OpenAI shut down ChatGPT Instant Checkout in March 2026, with broader industry attention moving to Google's UCP; treat it as a narrowing pilot rather than a converging standard.
...
| ---------------- | ------------------ | -------------------------------------------------------------------------------------- |
| MCP `2025-11-25` | Yes                | Stable spec, broad SDK support, and an official registry (revision `2026-07-28` in RC) |
| A2A `v1.0.1`     | Yes                | Production-grade — 150+ organizations, SDKs in five languages, multi-transport         |
| ACP (commerce)   | Pilots only        | Beta; deflating after the March 2026 Instant Checkout shutdown, momentum → Google UCP  |
