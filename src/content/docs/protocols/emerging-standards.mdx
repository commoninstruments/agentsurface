...
Complementary standards address workflows (Arazzo), observability (OpenTelemetry GenAI), and discovery (AGENTS.md, llms.txt, JSON-LD). Arazzo is stable at v1.1.0; OpenTelemetry GenAI semconv is useful but still marked Development. Publish discovery docs at well-known paths and use standards-based formats to avoid vendor lock-in.

- Arazzo 1.1.0: machine-readable multi-step workflows on OpenAPI and AsyncAPI descriptions
- OpenTelemetry GenAI: Development semantic conventions for agent observability
...
### Arazzo 1.1.0 (May 2026) — Multi-Step Workflow Orchestration

Arazzo defines machine-readable multi-step workflows layered on API descriptions such as OpenAPI and AsyncAPI. Where OpenAPI describes individual operations, Arazzo describes the correct sequence of calls, conditional branching, error handling, and success/failure paths.
...
Use Arazzo when your API involves sequences that agents should follow predictably. It reduces agent reasoning load and improves determinism. Version 1.1.0 adds AsyncAPI source description support and tighter reference-resolution guidance.

**Spec:** [Arazzo 1.1.0](https://spec.openapis.org/arazzo/latest.html)

### OpenTelemetry GenAI Semantic Conventions (Development)

OpenTelemetry's GenAI semantic conventions standardize how to instrument LLM calls and agentic workflows, but the current OpenTelemetry docs still mark them Development. As of semconv `v1.42.0` (2026-06-12) the GenAI conventions moved into a dedicated repository, `open-telemetry/semantic-conventions-genai`, which currently has **zero tagged releases** — the conventions are effectively unversioned. Metrics include request model, token counts, input/output content, system instructions, and tool call traces. Adoption by Traceloop, Langfuse, Datadog, and others makes this a practical observability baseline, but expect breaking changes and migration work before the conventions stabilize.
...
AGENTS.md is read by several coding-agent and IDE tools, including Cursor, Windsurf, Kilo, GitHub Copilot, JetBrains Codex, and others. It costs little to maintain and gives agents a predictable place to find project commands, boundaries, and context.
...
The IETF and adjacent standards groups have multiple drafts for agent discovery. Three approaches are worth tracking:
...
**Adopt production-useful standards now.** Use AGENTS.md, llms.txt, schema.org JSON-LD, and Arazzo workflows. Use OpenTelemetry GenAI semconv for observability, but expect breaking changes: the conventions remain Development and are unversioned in their new `semantic-conventions-genai` repository.
