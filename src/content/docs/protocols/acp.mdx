...
<Callout type="warn">
  **"ACP" is an overloaded acronym.** Three unrelated protocols have used it:

  1. **Agentic Commerce Protocol** (this page) — OpenAI + Stripe. Beta, latest spec `2026-04-17`.
  2. **Agent Client Protocol** — a Zed-originated standard for editor-to-agent communication, in its
     own org (`agentclientprotocol`), Apache-2.0, at stable protocol version 1 and very active.
     Adopters include Zed, JetBrains (natively), Neovim, and Emacs; a joint ACP Agent Registry
     (January 2026) lists roughly 50 agents (Gemini CLI, a Claude Code adapter, Codex CLI, Copilot,
     Goose). This is not a commerce protocol.
  3. **IBM ACP** (Agent Communication Protocol) — dead; absorbed into [A2A](/docs/protocols/a2a) in
     August 2025.
</Callout>
