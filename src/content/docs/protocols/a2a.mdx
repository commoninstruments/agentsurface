...
- OAuth 2.1 + DPoP for authentication between agents; Agent Cards can be cryptographically signed
- Works across org boundaries, different infrastructure
- Multi-transport: JSON-RPC, gRPC, and HTTP+JSON
- v1.0.1: production-grade — 150+ organizations, SDKs in 5 languages, an 8-state task lifecycle
...
Google initially developed and published the A2A specification in April 2025 as an open-source protocol for multi-agent coordination. In June 2025 it joined the Linux Foundation, and it now sits under the Agentic AI Foundation (AAIF) alongside MCP, making A2A a vendor-neutral standard. The spec reached v1.0.0 on 2026-03-12 and v1.0.1 on 2026-05-28 — production-grade, with 150+ participating organizations and SDKs across five languages. For current adopters and governance details, use the A2A project site rather than copying a vendor list into implementation docs.

IBM's separate "ACP" (Agent Communication Protocol) was absorbed into A2A in August 2025 and is dead as a standalone effort. See the [ACP page](/docs/protocols/acp) for the three unrelated protocols that share that acronym.
  ...
  "skills": [
    {
      ...
      "examples": ["Find business class flights from JFK to LHR for 2 passengers on March 15"]
    },
    {
      "id": "flight-book",
      "name": "Book Flight",
...
A task moves through a defined set of states. v1.0.1 specifies eight:
...
| `auth-required`  | Agent needs additional authorization before continuing |
| `completed`      | Task finished successfully                             |
| `failed`         | Task failed with an error                              |
| `canceled`       | Task was canceled by the client or timed out           |
| `rejected`       | Agent declined the task before starting work           |
...
The `skills` array in an AgentCard describes discrete capabilities the agent exposes. Unlike MCP tools (which are atomic functions), A2A skills represent multi-step tasks that may involve reasoning, user interaction, or cross-organizational workflows. Examples help orchestrators match natural-language tasks to the right skill.

## Spec & References

- [A2A v1.0.1 Specification](https://a2a-protocol.org/latest/specification/) — official A2A specification from the Linux Foundation
- [A2A Project Homepage](https://a2aproject.github.io/A2A/latest/) — documentation and resources
