...
Freshness note: last reviewed 2026-06-02. Multimodal model coverage, modality support, pricing, and benchmark positions move quickly; verify current provider docs before relying on a named model or ranking.

## Summary

Use multimodal embeddings when documents include images (PDFs, slides, charts), video archives, or cross-modal queries such as "find documents with charts like this image." Verify current modality support before choosing a model: Gemini embedding models, Voyage Multimodal, Cohere Embed, CLIP, and SigLIP move at different speeds across text, image, video, audio, and code.

**Key takeaways:**

- **Gemini embedding models:** Check current Google model docs for modality support and benchmark claims.
...
| ------------------------- | ------------------- | ------------------ | ----------------------------- | ------------------------ | ------------------------------ |
| **Gemini Embedding 2**    | Verify current docs | 768                | ~$0.02/1M                     | Verify current benchmark | Google API                     |
...
```typescript
async function embedVideoFrames(videoPath: string, intervalSeconds: number = 5) {
...
async function paliRetrievePDFs(query: string, pdfPaths: string[]) {
  ...
  const scores = pdfImages.map((_, idx) => maxSim(queryTokens, pdfTokenEmbeds[idx]));
...
// Schema
export const multimedialDocs = pgTable("multimedia_docs", {
...
// Insert
async function storeMultimodalDoc(docId: string, imageBlob: Buffer, metadata: Record<string, any>) {
...
async function searchMultimedial(query: string) {
  ...
  const results = await db
    ...
    .orderBy(sql`embedding <-> ${pgvector.toSql(queryEmbed.data[0].embedding)}`)
    .limit(10);
