...
Dense embeddings transform text into fixed-length vectors suitable for nearest-neighbor search. Several open-source models now beat some commercial APIs on specific MTEB tasks, but leaderboard rank is not a substitute for a domain eval. Choose based on cost sensitivity, latency SLA, privacy boundary, and corpus shape. **Matryoshka embeddings** (supported by OpenAI 3-large and Cohere v4) let you store full dimensions but truncate to 256–512 at query time for cost savings without recomputing.

**Key takeaways:**

- Commercial APIs (OpenAI, Voyage, Cohere) are integrated but expensive ($0.01–0.12 per 1M tokens).
- Open-source (BGE-M3, E5-Mistral, Nomic) can perform well on MTEB; zero API costs, variable latency.
...
| **BGE-M3**                        | 1,024              | Free      | 65.1                     | 10–20ms (GPU)       | Open-source; multilingual                             |
...
| **Google Gemini Embedding 2**     | 768                | ~$0.02/1M | Verify current benchmark | 100–300ms (API)     | All-modality family (text, image, video, audio, code) |
...
On domain-specific corpora, compare specialized and general models with recall@k or nDCG before switching.
    ...
    const embedded = await pRetry(
        ...
        }),
      {
        ...
        onFailedAttempt: (error) => {
          console.warn(`Embedding batch attempt ${error.attemptNumber} failed.`, error.message);
        },
      },
    );
...
async function embedLargeDataset(inputFile: string, outputFile: string, batchSize: number = 100) {
  const lines = fs
