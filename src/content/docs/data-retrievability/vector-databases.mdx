...
Choose serverless vector databases for hands-off operations, pgvector for cost-sensitive Postgres-backed systems, Qdrant for fine-grained control, or Weaviate for native hybrid search. Timescale's 50M-vector benchmark reported lower p95 latency and lower self-hosted cost for pgvector + pgvectorscale than Pinecone s1, but vendor pricing and workloads vary.

**Key takeaways:**

- **Pinecone serverless:** Managed, hybrid + rerank, auto-scaling.
- **pgvector + pgvectorscale:** Self-hosted PostgreSQL path for teams that can operate Postgres.
...
// Hybrid: FTS + vector
async function hybridSearch(query: string, queryEmbedding: number[], limit: number = 10) {
  ...
  return Array.from(merged.values()).sort(
    (a, b) =>
      (a.ftsRank ?? 999) + (a.vectorRank ?? 999) - ((b.ftsRank ?? 999) + (b.vectorRank ?? 999)),
  );
}
```
...
- **Pinecone:** estimate from current storage, read-unit, write-unit, and rerank pricing; do not treat old cost examples as stable.
