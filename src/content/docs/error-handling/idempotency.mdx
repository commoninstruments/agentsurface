...
- Prefer UUIDv7 keys — the embedded timestamp lets old keys expire naturally
...
Idempotency keys handle retries safely. `Retry-After` headers tell the agent _when_ to retry. Both are needed:
...
- [Agent extensions](/docs/error-handling/rfc-9457#recommended-extensions-for-agent-apis) — `is_retriable` flag
