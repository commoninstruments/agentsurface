...
For higher-assurance contexts, run returned text through your approved prompt-injection or content-filtering layer before returning it to the agent. Document the scanner, policy version, and action taken on failure.
...
This pattern is optional — it ensures the agent is executing precisely the operation it previewed, not a new version of the command constructed with potentially different parameters.
...
| 3     | Dry-run plus response sanitization to defend against prompt injection embedded in API data. The full request→response loop is defended. |
