...
Crawler economics are shifting alongside verification. Cloudflare's pay-per-crawl experiment pivoted to "Pay Per Use" on 2026-07-01: publishers are compensated when their content is cited or appears in AI answers, rather than per crawl, with an x402-settled Monetization Gateway in waitlist. Blocking versus allowing AI bots is no longer purely a control decision — for some publishers it is becoming a pricing decision.
