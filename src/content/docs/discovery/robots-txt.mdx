...
robots.txt now controls two distinct bot types: training crawlers (index for model training) and retrieval bots (fetch during inference). Blocking GPTBot does not prevent ChatGPT-User from reading your docs. This two-decision model lets retrieval bots fetch public docs while disallowing listed training crawlers that honor robots.txt. Recommended: block training crawlers, allow retrieval bots.
...
User-agent strings are trivially spoofable, so log-based verification only goes so far. Web Bot Auth is the emerging answer: signature-based verification that identifies bot traffic cryptographically rather than by user-agent, building on RFC 9421 HTTP Message Signatures so an accountable agent signs its requests and you verify them against a published key directory. Cloudflare has run it in production since March 2026 (folded into Verified Bots). It lets you admit known "good bots" while blocking automation that merely claims a friendly user-agent. See [Agent Identity](/docs/authentication/agent-identity) for how request signing fits the broader agent-authorization model.

Crawler economics are shifting alongside verification. Cloudflare's pay-per-crawl experiment pivoted to "Pay Per Use" on 2026-07-01: publishers are compensated when their content is cited or appears in AI answers, rather than per crawl, with an x402-settled Monetization Gateway in waitlist. Blocking versus allowing AI bots is no longer purely a control decision — for some publishers it is becoming a pricing decision. See [Agentic Commerce Protocols](/docs/protocols/agentic-commerce) for how the 402 payment flow works.
