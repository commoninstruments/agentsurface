...
```json
{
  ...
  "capabilities": ["api", "mcp", "llms-txt", "openapi", "webhooks"],
  "api": {
...
### WebMCP — Tools Exposed In-Browser

The well-known MCP files above advertise a _remote_ MCP server. WebMCP is the in-browser counterpart: a W3C proposal that lets a site expose callable tools to an agent running inside the page through a `navigator.modelContext` browser API, rather than pointing at a separate server. It shipped a Chrome 149 origin trial, so you can experiment behind a flag, but the API surface is unstable and there is no cross-browser commitment yet. Track it on the [emerging standards watchlist](/docs/protocols/emerging-standards); publish a server card for anything you need agents to reach today.
