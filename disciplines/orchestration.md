...
Orchestration is the art of scaling from a single agent with tools to multi-agent systems. The decision hinges on _scope, state, and specialization_: Does the problem require multiple loosely coupled specialists, or can one generalizer reason through it? Single agents scale up to ~10 tools with shared state; multi-agent systems become necessary when specialists must operate in parallel, maintain isolated state, or have incompatible reasoning styles.
...
When you do split, partition by _problem domain_, not by task type.
...
Within a single agent, tool invocation is a _loop_:
